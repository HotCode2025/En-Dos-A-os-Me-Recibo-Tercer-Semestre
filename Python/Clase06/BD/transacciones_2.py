...
try: #Esto es para intentar ejecutar el bloque de código que se encuentra dentro del bloque try, y capturar cualquier excepción que pueda ocurrir durante la ejecución del programa.
    conexion.autocommit = False #Esto es para desactivar el autocommit de la conexión, lo que significa que los cambios realizados en la base de datos no se confirmarán automáticamente, sino que se tendrán que confirmar manualmente utilizando el método commit de la conexión. Inicia la transacción.
    ...
    valores = ('Jorge', 'Caparros', 'jorgecaparros@gmail.com') #Esto es para definir el valor de los placeholders de la sentencia SQL.
    ...
    valores = ('Juan Carlos', 'Perez', 'juanperez@gmail.com', 1) #Esto es para definir el valor de los placeholders de la sentencia SQL.
    cursor.execute(sentencia, valores) #Esto es para ejecutar la sentencia SQL en la base de datos, pasando los valores como parámetros.
    ...
    conexion.commit() #Esto es para confirmar los cambios realizados en la base de datos utilizando el método commit de la conexión. Finaliza la transacción.
